...
struct file_operations irq_button_drv_file_st = {
	.owner = THIS_MODULE,
	.read = button_read,
	.write = button_write,
	.open = button_open,
...
static int __init irq_button_drv_init(void)
{
	int ret;
	ret = 0;
	printk("%s:%s\n", __func__, __FILE__);
	...
	btn->major = register_chrdev(0, "irq_button_drv", &irq_button_drv_file_st);
	if (0 > ret) {
		ret = btn->major;
		goto register_chrdev_error;
	}
	
	btn->btn_cls = class_create(THIS_MODULE, "irq_button_drv_class");
...
struct class_device *class_device_create(struct class *cls,
	...
	btn->btn_cls_dev = class_device_create(btn->btn_cls,
			NULL, MKDEV(btn->major, 0), NULL, "irq_button_class_dev");
...
class_create_error:
	unregister_chrdev(btn->major, "thrid_drv");
...
static void __exit irq_button_drv_exit(void)
{
