...
struct file_operations async_drv_file_st = {
	.owner = THIS_MODULE,
	.read = button_read,
	.write = button_write,
	.open = button_open,
...
static int __init async_drv_init(void)
{
	int ret;
	ret = 0;
	printk("%s:%s\n", __func__, __FILE__);
	...
	btn->major = register_chrdev(0, "async_drv", &async_drv_file_st);
	if (0 > ret) {
		ret = btn->major;
		goto register_chrdev_error;
	}
	
	btn->btn_cls = class_create(THIS_MODULE, "async_drv_class");
...
class_create_error:
	unregister_chrdev(btn->major, "async_drv");
...
static void __exit async_drv_exit(void)
{
