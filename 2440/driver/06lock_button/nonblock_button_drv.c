...
struct file_operations nonblock_button_file_st = {
	.owner = THIS_MODULE,
	.read = button_read,
	.write = button_write,
	.open = button_open,
...
static int __init nonblock_button_init(void)
{
	int ret;
	ret = 0;
	printk("%s:%s\n", __func__, __FILE__);
	...
	btn->major = register_chrdev(0, "nonblock_button", &nonblock_button_file_st);
	if (0 > ret) {
		ret = btn->major;
		goto register_chrdev_error;
	}
	
	btn->btn_cls = class_create(THIS_MODULE, "nonblock_button_class");
...
struct class_device *class_device_create(struct class *cls,
	...
	btn->btn_cls_dev = class_device_create(btn->btn_cls,
			NULL, MKDEV(btn->major, 0), NULL, "nonblock_button_dev");
...
class_create_error:
	unregister_chrdev(btn->major, "nonblock_button");
...
static void __exit nonblock_button_exit(void)
{
...
module_init(nonblock_button_init);
module_exit(nonblock_button_exit);
