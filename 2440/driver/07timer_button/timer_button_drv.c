...
int button_open(struct inode *inode, struct file *fp)
{
	...
	fp->private_data = btn;
	if ((fp->f_flags & O_NONBLOCK)) {
		if (down_trylock(&btn->sem))
			return -EAGAIN;
		/* 当获得信号量时，则通过down_trylock返回值为0的隐式分
		 * 跳出当前分支结构，并且将down_trylock写在分支内部起到
		 * 只让标志为是非阻塞时才进行信号量操作，起到避免
		 * 信号量重复执行的作用 */
	} else {
...
struct file_operations timer_button_file_st = {
	.owner = THIS_MODULE,
	.read = button_read,
	.write = button_write,
	.open = button_open,
...
irqreturn_t button_irq(int irq, void *data)
{
	...
	btn->desc_flag = (struct pin_desc*)data;
	btn->timer.data = (unsigned int)data;
	mod_timer(&btn->timer, jiffies + HZ / 100);
	return IRQ_HANDLED;
}

static int __init timer_button_init(void)
{
	int ret;
	ret = 0;
	printk("%s:%s\n", __func__, __FILE__);
	...
	btn->major = register_chrdev(0, "timer_button", &timer_button_file_st);
	if (0 > ret) {
		ret = btn->major;
		goto register_chrdev_error;
	}
	
	btn->btn_cls = class_create(THIS_MODULE, "timer_button_class");
...
struct class_device *class_device_create(struct class *cls,
	...
	btn->btn_cls_dev = class_device_create(btn->btn_cls,
			NULL, MKDEV(btn->major, 0), NULL, "timer_button_dev");
	...
	btn->timer.expires = jiffies;
	btn->timer.function = button_timer_func;
	add_timer(&btn->timer);
...
class_create_error:
	unregister_chrdev(btn->major, "timer_button");
...
static void __exit timer_button_exit(void)
{
	del_timer(&btn->timer);
...
module_init(timer_button_init);
module_exit(timer_button_exit);
