...
{
	w->wtcon = (99 << 8) | (2 << 3) | (1 << 2);
	iowrite32(w->wtcon, w->v + WTCON);

	w->wdtcnt = 15625 * 3;
	iowrite32(w->wdtcnt, w->v + WTCNT);

	/* 时间到了就会将data中的数据复制给count寄存器 */
	iowrite32(w->wdtcnt, w->v + WTDAT);
...
#endif
irqreturn_t irq_wdt_handler(int irq, void *data)
{
	struct wdt_st *w = data;
	printk("%s:%d %s,data:%p\n",
		__FILE__, __LINE__,
		__func__, data);
	/* 随便写个值就能清中断，内部中断需要手动清中断，外部中断内核会帮助清中断，看门狗属于内部 */
	writel(1, w->v + WTCLRINT);
	return IRQ_HANDLED;
}
...
#endif
	ret = request_irq(IRQ_WDT,
	irq_wdt_handler,
	0, /* 内部中断不需要触发电平是那个阶段判断 */
	"mywdtirq", wdt/* 处理函数获取数据指针 */);
	if (ret < 0) {
