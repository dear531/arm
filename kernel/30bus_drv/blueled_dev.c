...
/* 当设备注销的时被调用 */
void blue_led_release(struct device *dev)
{
	printk("blue led unrigister\n");
}
...
struct led_device blue = {
	...
	.dev = {
		...
		.bus = &ledbus.bus, 
		/* 注册设备的名字,
		 * 以此名称bluedev挂在其所挂载的变量ledbus所在的目录/sys/bus/ledbus/
		 * 下的设备目录devices/下
		 * /sys/bus/ledbus/devices/bluedev/ */
		.init_name = "bluedev",
		/* 当设备注销的时被调用 */
		.release = blue_led_release,
	},
...
{
	printk("%s:%d\n", __func__, __LINE__);
	return device_unregister(&blue.dev);
}
